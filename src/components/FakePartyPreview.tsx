...
const TODAY_PCT = Math.min(
  Math.round((TODAY_TOTAL_STEPS / TARGET_TOTAL) * 100),
  100,
);
...
function FakeCheerTicker() {
  ...
  const handleEnterEnd = () => {
    const next = (idxRef.current + 1) % FAKE_CHEERS.length;
    setIdx(next);
    idxRef.current = next;
    setTransitioning(false);
  };

  const curr = FAKE_CHEERS[idx];
  const next = FAKE_CHEERS[(idx + 1) % FAKE_CHEERS.length];
    ...
    <div className="bg-white rounded-2xl shadow-sm px-4 h-9 flex items-center gap-2.5 overflow-hidden">
      ...
      <div className="flex-1 relative h-5 overflow-hidden">
        <div
          ...
          style={
            transitioning
              ? { animation: "fakeExitUp 0.38s ease-in-out forwards" }
              : undefined
          }
        >
          <span
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <div className="h-4 flex items-center justify-center">
        {!m.inactive7 && (
          <p
            className={`text-[9px] font-bold ${m.isActive ? "text-emerald-500" : "text-gray-400"}`}
          >
...
function FakePartyDetailOverlay({ onClose }: { onClose: () => void }) {
  ...
  const sendCheer = () => {
    ...
    if (!text) return;
    setCheers((prev) =>
      [...prev, { id: `u-${Date.now()}`, nickname: "나", text }].slice(-20),
    );
    setCheerInput("");
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-bg flex flex-col overflow-hidden">
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-5 pt-5 pb-3 bg-bg">
        <button
          onClick={onClose}
          aria-label="뒤로가기"
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-3xl shadow-sm px-4 py-3 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {["🌅 새벽", "👥 9/10명", "👟 10,000보"].map((tag) => (
              <span
                key={tag}
                className="text-[10px] text-gray-500 font-semibold bg-gray-50 px-2 py-1 rounded-full"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex items-center justify-between">
            <p className="text-xs font-bold text-gray-700">
              <span className="text-orange-500">
                {TODAY_TOTAL_STEPS.toLocaleString()}
              </span>
              {" / "}
          ...
          <div className="flex items-center gap-3 bg-amber-50 rounded-2xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-amber-500">
                {mvp.todaySteps.toLocaleString()}보
              </p>
            </div>
          </div>
        </div>
        ...
        {/* 응원 티커 */}
        <FakeCheerTicker key={cheers.length} />
      ...
      <div className="bg-white shadow-sm p-4">
        <button
          onClick={() =>
            showToast("데모 파티예요! 실제 파티를 만들어보세요 🎉")
          }
          className="w-full py-3 rounded-2xl text-sm font-extrabold transition active:scale-95 bg-primary text-white"
        >
          🎉 참가하기
        </button>
      </div>
    ...
    <>
      <div className="relative">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, var(--color-primary)22, var(--color-secondary)22)",
            color: "var(--color-primary)",
          }}
        >
          <span>✨</span>
      ...
      {showDetail && (
        <FakePartyDetailOverlay onClose={() => setShowDetail(false)} />
      )}
    </>
  );
}
