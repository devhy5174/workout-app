...
#### 커뮤니티 인증글 하루 1회 제한
- 오늘 이미 인증한 경우 새 글 대신 기존 글 수정 처리
- 수정 시 현재 시점 걸음수로 자동 갱신
- 쓰기 버튼 UI: 미인증 시 `+`, 인증 완료 시 ✅ + "오늘 인증 완료! 수정하기"
- 모달 제목·버튼도 수정 모드 반영
- 걸음수 0보 상태에서 인증 시도 시 운동 유도 팝업 표시

#### 이벤트 보상 해금 팝업
- 고정 streak 이벤트 조건 달성 시 Step 페이지에서 즉시 해금 팝업
- 개인/파티 이벤트 달성 시 운동 종료 후 팝업 (RPC 전후 grant 비교)
- `autoGrantFixedEvent` → boolean 반환으로 신규 지급 여부 판단

#### 이벤트 기능 개선
- 이벤트 보상 자동지급 후 `refreshGrants()` 호출로 UI 즉시 갱신
- 이벤트 한정 보상 아이템 granted 시 선택 가능 + 해금 스타일 반영
- personal 이벤트 진행률을 이벤트 기간 내 걸음수 기준으로 수정

#### 알림 권한 안내
- 홈 진입 시 알림 권한 거부 상태면 세션 1회 경고 팝업
- 알림 설정 페이지에 기기 알림 권한 필수 안내 카드 추가 (운동 트래커 연관)
- 알림 권한 거부 시 기기 설정 안내 배너 표시

### 버그 수정

#### 스트릭 이벤트 카운트 오류
- 이벤트 시작 전 운동 기록이 스트릭 카운트에 포함되던 버그 수정
- `calcStreakSince(dates, sinceDate)` 추가 — 이벤트 startDate 이전 날짜 제외

#### 파티 랭킹
- 파티 목표가 하루 기준(보/인)이므로 랭킹도 주간 누적 → 오늘 걸음수 기준으로 수정

#### 커뮤니티 posts 수정 권한
- `community_posts` 테이블 UPDATE RLS 정책 누락으로 406 에러 발생
- 본인 글만 수정 가능한 RLS 정책 추가 (`supabase/community_posts_rls.sql`)

#### 말풍선 꼬리 z-index
- 말풍선 꼬리(마름모)가 본체 위에 올라오던 문제 수정
- 본체 `z-10`, 꼬리 `relative`로 꼬리가 뒤로 들어가는 구조로 수정

#### 창립 멤버 말풍선
- `premium: false`로 변경 (프리미엄 전용 아님, 이벤트 한정)
- `darkText: true` 추가 (골드 배경 → 검정 텍스트)
- 프리미엄 탭 미리보기에서 `exclusive` 버블 제외
- StreakChallengeCard 보상 미리보기에 실제 버블 컬러·반짝이 적용

### 기타

#### UI 개선
- 파티 페이지 예시 파티 0개일 때만 표시
- 홈 Top3 "전체 N명" 텍스트 제거
- 운동 알림 설명에서 "트래커 알림 전체" 문구 제거 (트래커는 시스템 권한으로 동작)
