...
- 스플래시 화면: 앱 아이콘 + "함께걸어요" 텍스트 (`splash_layout.xml`)
- 앱 브랜드 소개 슬라이드 (인트로)
...
| 유형 | 메인 카드 | 보조 3칸 |
|------|-----------|----------|
| 🚶 산책러 | 걸음수 (크게) | 칼로리 · 거리(GPS 배지) · 시간 |
| 🚶‍♂️ 파워워커 | 거리 + 걸음수 (2열) | 페이스 · 시간 · 칼로리 |
| 🏃 러너 | 거리 + 걸음수 (2열) | 페이스 · 시간 · 칼로리 |
| 🏔️ 등산가 | 거리 + 걸음수 (2열) | 시간 · 페이스 · 칼로리 |

모든 유형에서 GPS/추정 배지가 노출됩니다. 산책러는 거리 보조 카드에, 나머지 유형은 메인 거리 카드에 표시됩니다.  
결과 팝업 및 운동 상세 페이지의 GPS 경로 지도는 `route_points.length >= 2` 시 모든 유형에서 동일하게 표시됩니다.
...
| 스케줄명 | 시각 (KST) | 대상 조건 | 제목 | 본문 |
|----------|-----------|-----------|------|------|
| `activity-reminder` | 오후 12시 | 최근 7일 활성 & 오늘 미운동 | ☀️ 오늘의 첫 걸음, 저랑 같이 시작해요! | 아직 오늘 운동 기록이 없어요. 10분만 움직여도 하루가 달라진답니다 🐾 |
| `diet-lunch-reminder` | 오후 1시 | 최근 7일 활성 유저 전체 | 🥗 밥 먹고 저랑 10분만 걸어요! | 식사 후 가볍게 걸으면 혈당도 잡히고 기분도 좋아져요 🌿 |
| `diet-dinner-reminder` | 오후 6시 | 오늘 운동 완료한 유저 | 오늘 운동 성공! 저녁 식단 가이드 🌙 | 오늘 {N}kcal 소모했어요. 맞춤 저녁 식단을 확인해보세요! |
| `streak-warning` | 오후 8시 | 최근 3일 활성 & 오늘 미운동 (스트릭 있음) | 🔥 {N}일 스트릭, 오늘 끊기면 아깝잖아요! | 딱 10분만 움직이면 {N}일 스트릭 지킬 수 있어요. 저 준비됐어요! 같이 나가요! |
| `streak-warning` | 오후 8시 | 최근 3일 활성 & 오늘 미운동 (스트릭 없음) | 🚶 저랑 오늘 첫 기록 만들어볼까요? | 10분만 움직여도 오늘의 기록이 시작돼요. 함께해요! |
...
파티 시작 / 파티원 합류 이벤트 발생
  ...
  → notify-fcm Edge Function 호출
  → Google FCM HTTP v1 API로 발송
  → 기기 잠금화면·알림창에 즉시 표시

[목표 달성 · 스트릭 경고 · 식단 리마인더]
앱 내 이벤트 발생 → createNotification() 호출
  → notifications 테이블 INSERT (인앱 알림)
  → notify-fcm Edge Function 호출 (FCM 푸시)
  → 기기 잠금화면·알림창에 즉시 표시
```
...
**Edge Function 배포 시 필수 주의사항**

알림 관련 함수는 반드시 `--no-verify-jwt` 플래그로 배포해야 합니다.  
`config.toml`의 `verify_jwt = false` 설정은 Supabase CLI가 배포 시 무시하며, 플래그를 생략하면 서버-서버 호출이 `401 UNAUTHORIZED_INVALID_JWT_FORMAT`으로 차단됩니다.

```bash
npx supabase functions deploy notify-fcm          --project-ref <REF> --no-verify-jwt
npx supabase functions deploy notify-scheduled    --project-ref <REF> --no-verify-jwt
npx supabase functions deploy notify-push         --project-ref <REF> --no-verify-jwt
npx supabase functions deploy notify-party-joined --project-ref <REF>
npx supabase functions deploy notify-party-start  --project-ref <REF>
```

**FCM payload 요구사항 (Android 잠금화면 표시)**
...
- `android.priority` 는 반드시 대문자 `"HIGH"` — 소문자 `"high"` 는 FCM HTTP v1 API에서 무효 처리됨
- `notification_priority: "PRIORITY_HIGH"` 없으면 헤즈업·잠금화면 표시 안 됨
- `channel_id: "default"` 는 `MainActivity.java`의 `IMPORTANCE_HIGH` 채널과 일치

**테스트 방법**

앱이 포그라운드 상태일 때는 Android 표준 동작으로 시스템 알림이 자동 표시되지 않습니다.  
반드시 **앱 완전 종료 또는 홈 버튼으로 백그라운드 전환 후** 테스트하세요.
